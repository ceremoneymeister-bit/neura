...
### ⚡ Anti-Flicker: Scroll-Анимации (Vite SPA)

Framer Motion `whileInView` + stagger часто мерцает. Надёжная альтернатива — CSS transitions + `useInView`:
...
**Ключевое:** один `useInView` ref на секцию, CSS `transitionDelay` для каскада, НЕ отдельный `whileInView` на каждом элементе
